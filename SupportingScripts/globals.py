#!/usr/bin/env python3

"""
Containers

This class contains the location for every contianer
and the location to drive to for each container
"""
class Containers():
    A1 = [[9*2.54,13*2.54],[15*2.54,13*2.54],[21*2.54,13*2.54],[27*2.54,13*2.54],[33*2.54,13*2.54],[39*2.54,13*2.54],
          [9*2.54,35*2.54],[15*2.54,35*2.54],[21*2.54,35*2.54],[27*2.54,35*2.54],[33*2.54,35*2.54],[39*2.54,35*2.54]]
    A2 = [[9,37],[15,37],[21,37],[27,37],[33,37],[39,37],
          [9,59],[15,59],[21,59],[27,59],[33,59],[39,59]]
    
    B1 = [[69,13],[75,13],[81,13],[87,13],[93,13],[99,13],
          [69,35],[75,35],[81,35],[87,35],[93,35],[99,35]]
    B2 = [[69,37],[75,37],[81,37],[87,37],[93,37],[99,37],
          [69,59],[75,59],[81,59],[87,59],[93,59],[99,59]]
    
    C1 = [[9,61],[15,61],[21,61],[27,61],[33,61],[39,61],
          [9,83],[15,83],[21,83],[27,83],[33,83],[39,83]]
    C2 = [[9,85],[15,85],[21,85],[27,85],[33,85],[39,85],
          [9,107],[15,107],[21,107],[27,107],[33,107],[39,107]]
    
    D1 = [[69,61],[75,61],[81,61],[87,61],[93,61],[99,61],
          [69,83],[75,83],[81,83],[87,83],[93,83],[99,83]]
    D2 = [[69,85],[75,85],[81,85],[87,85],[93,85],[99,85],
          [69,107],[75,107],[81,107],[87,107],[93,107],[99,107]]
    
    CONTAINER_SIZE = 4 * 2.54
    ROBOT_SIZE = 10.25 * 2.54
    DISTANCE_BETWEEN_ROBOT_AND_CONTAINER = 0.5 *2.54
    
    DISTANCE_BETWEEN_CENTERS = CONTAINER_SIZE/2 + ROBOT_SIZE/2 + DISTANCE_BETWEEN_ROBOT_AND_CONTAINER

# Contants for the Center Locations
HOME_A_X = 0
HOME_A_Y = 0
    
FULFILLMENT_CENTER_B_X = 100 * 2.54
FULFILLMENT_CENTER_B_Y = 0

FULFILLMENT_CENTER_C_X = 0
FULFILLMENT_CENTER_C_Y = 100 * 2.54

FULFILLMENT_CENTER_D_X = 100 * 2.54
FULFILLMENT_CENTER_D_Y = 100 * 2.54

A_B_HALL_Y = 21*2.54